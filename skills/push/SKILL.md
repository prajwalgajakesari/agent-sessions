...
1. **Check the preflight.** If it ends in `STATUS: error`, explain the `REASON` and `WARNINGS` to the user and stop. The usual fix is running `/sessions:init` in this repo. Otherwise note `SESSION_DIR`, `TRANSCRIPT`, `TITLE_DEFAULT` and `REPUSH`.

2. **Write the summary.** Read `${CLAUDE_PLUGIN_ROOT}/templates/summary.md` and write a filled-in copy to `<SESSION_DIR>/summary.md`, creating the directory if needed. Use the `OUTLINE` from the preflight plus your own memory of this conversation. Rules:
   - Hard cap of 120 lines and 6 KB. Export rejects anything larger.
   ...
   - If `REPUSH: yes`, read the existing `summary.md` first and revise it instead of starting over.

3. **Export.** Run with the Bash tool:
   ```
   "${CLAUDE_PLUGIN_ROOT}/scripts/sessions.sh" export --transcript "<TRANSCRIPT>" --out "<SESSION_DIR>"
   ```
   Append `--include-results` only if the user asked for it. On `STATUS: error` show the `REASON` and stop.
...
5. **Commit and push.** Run with the Bash tool:
   ```
